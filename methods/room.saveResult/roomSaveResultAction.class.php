...
class roomSaveResultAction extends baseAction
{
    ...
    public function execute()
    {
        ...
        if ($this->room_type == 3)
        {
            ...
            if (!empty($deletemsgId) || empty($isMsg_RoomId_check))
            { //&& empty($isMsg_RoomId_check)
        ...
        {
            $this->setResponse('CUSTOM_ERROR', array(
        ...
        if ($this->winStatus > - 1)
        {
            ...
            if ($oppBattleStatus == 1)
            {
                $oppdestroyed_tower = $this->opponent_circlet;
            }
            elseif ($oppBattleStatus == 2)
            {
                $oppdestroyed_tower = $this->circlet;
            }
            elseif ($oppBattleStatus == 3)
            {
                $oppdestroyed_tower = $this->opponent_circlet;
            }
            $matchStatusRewardBattleHistOpp = $rewardLib->getMasterMatchStatusRewardForStadiumByTower($oppBattleStatus, $oppdestroyed_tower, $oppUserData['master_stadium_id']);
            if(empty($matchStatusRewardBattleHistOpp) || $matchStatusRewardBattleHistOpp == ""){
              ...
              //$matchStatusReward = $rewardLib->getMasterMatchStatusRewardForStadium($this->winStatus, $maxStadiumId);
              $matchStatusRewardBattleHistOpp = $rewardLib->getMasterMatchStatusRewardForStadiumByTower($oppBattleStatus, $oppdestroyed_tower, $oppMaxStadId);
            }
            ...
            if (!empty($roomUserActive))
            {
                        ...
                        if ($qUserStadiumv['slide_count'] >= $qUserStadiumv['slide_maxvalue'])
                        {
                            ...
                            else
                            {
                                ...
                                if ($qUserStadiumv['slide_count'] != $qUserStadiumv['slide_maxvalue'])
                                {
                                    if ($user['master_stadium_id'] <= 5)
                                    {
                                        $questLib->updateQuestInventory($questData['quest_id'], $this->userId, array(
                                            'slide_count' => $user['master_stadium_id']
                                        ));
                                    }
                                    elseif ($user['master_stadium_id'] >= 5)
                                    {
                                    ...
                                    {
                                        $questLib->updateQuestInventory($questData['quest_id'], $this->userId, array(
                                            'slide_count' => $questData['slide_count'] + 1
                                        ));
                                    }
                                }
                            }
                ...
                if ($this->opponentId > - 1)//$this->opponentId 
                {
                    $opponentUser = $userLib->getUserDetail($this->battle_opp_id);
                    ...
                    $opponentMatchStatusReward = $rewardLib->getMasterMatchStatusRewardForStadiumByTower($oppBattleStatus, $oppdestroyed_tower, $opponentUser['master_stadium_id']);
                    //$opponentMatchStatusReward = $rewardLib->getMasterMatchStatusRewardForStadiumByTower(BATTLE_LOST_STATUS, $this->opponent_circlet, $opponentUser['master_stadium_id']);
                    ...
                    $opponentParamList['master_stadium_id'] = $masterStadium['master_stadium_id'];
                      ...
                      $roomLib->updateWaitingRoomForPlayerResult($this->roomId, $this->opponentId, array(
                          'win_status' => BATTLE_LOST_STATUS
                      ));
                      $roomLib->updateWaitingRoomStatus($this->roomId, $this->opponentId, array(
                          'status' => CONTENT_CLOSED
                      ));
                   
                    $userLib->updateUser($this->battle_opp_id, $opponentParamList);
                }

                //Update Waiting room and user detail.
                $roomLib->updateWaitingRoomForPlayerResult($this->roomId, $this->userId, $roomParamList);
                $roomLib->updateWaitingRoomStatus($this->roomId, $this->userId, array(
                    'status' => CONTENT_CLOSED
                ));
                ...
                if (!empty($opponentUser) && $opponentUser['is_ai'] == CONTENT_ACTIVE)
                {
                    if ($this->winStatus == BATTLE_WON_STATUS) $aiBattleStatus = BATTLE_LOST_STATUS;
                    else $aiBattleStatus = BATTLE_WON_STATUS;

                    $roomLib->updateWaitingRoomStatus($this->roomId, $opponentUser['user_id'], array(
                        'status' => CONTENT_CLOSED,
                        'win_status' => $aiBattleStatus
                    ));
                    $userLib->updateUser($opponentUser['user_id'], array(
                        'is_ai_available' => CONTENT_ACTIVE
                    ));
                }
            }
        }
        ...
        else
        {
            ...
            {
                $questLib->updateQuestInventory($questData['quest_id'], $this->userId, array(
                    'win_count' => $questData['win_count'] + 1,
                    'match_count' => $questData['match_count'] + 1
                ));
            }
            else
            {
        ...
        if ($this->winStatus == BATTLE_WON_STATUS) $oppBattleStatus = BATTLE_LOST_STATUS;
        elseif ($this->winStatus == BATTLE_DRAW_STATUS) $oppBattleStatus = BATTLE_WON_STATUS;
        else $oppBattleStatus = BATTLE_WON_STATUS;
        ...
        print_log("------------ opp id ----------");
        print_log($this->battle_opp_id);
        $oppUserData = $userLib->getUserDetail($this->battle_opp_id);
        $oppRelics = $oppUserData['relics'];
        $user = $userLib->getUserDetail($this->userId);
        $uRelics = $user['relics'];

       /* $checkuserValid=$roomLib->getBattleHistResultByUser($this->userId,$this->roomId);
        if(empty($checkuserValid) || $checkuserValid<1){*/
          $roomLib->deleteAllBattleHistoryByRoomAndUser($this->userId,$this->battle_opp_id,$this->roomId);
          $battleId = $userLib->insertBattleHistory(array(
              ...
              'user_winstatus' => $this->winStatus,
              'opponent_winstatus' => $oppBattleStatus,
              'user_relics_bonus' => (empty($roomUserActive) ? 0 : $matchStatusReward['relics']),
              'opp_relics_bonus' => (empty($roomUserActive) ? 0 :$matchStatusRewardBattleHistOpp['relics']),
              'user_stadium' => $user['master_stadium_id'],
              'opp_stadium' => $oppUserData['master_stadium_id'],
              'created_at' => date('Y-m-d H:i:s') ,
              'userDeckLst' => json_encode($usrdeckList) ,
          ...
          /*$checkOppuserValid=$roomLib->getBattleHistResultByOppUser($this->userId,$this->roomId);
          if(empty($checkOppuserValid) || $checkOppuserValid<1){*/
            $opponentUser = $userLib->getUserDetail($this->battle_opp_id);
            if (!empty($opponentUser) && $opponentUser['is_ai'] != CONTENT_ACTIVE)
            {
              $roomLib->deleteAllBattleHistoryByRoomAndUser($this->battle_opp_id,$this->userId,$this->roomId);
                $battleOpponentId = $userLib->insertBattleHistory(array(
                    ...
                    'user_winstatus' => $oppBattleStatus,
                    'opponent_winstatus' => $this->winStatus,
                    'user_relics_bonus' => (empty($roomUserActive) ? 0 :$matchStatusRewardBattleHistOpp['relics']),
                    'opp_relics_bonus' => (empty($roomUserActive) ? 0 : $matchStatusReward['relics']),
                    'user_stadium' => $oppUserData['master_stadium_id'],
                    'opp_stadium' => $user['master_stadium_id'],
                    'userDeckLst' => json_encode($usrdeckList) ,
                    'oppDeckLst' => json_encode($oppdeckList) ,
